...
export default function HomePage() {
    ...
    useEffect(() => {
        if (search.search.length > 0) {
            setPokemons(pokemons.results.filter(el => el.name.toLowerCase().includes(search.search.toLowerCase())))
        } else {
        ...
        <section className='md:p-10'>
            <div className='container bg-cyan-400 sm:w-[70%] mx-auto p-3 rounded-md'>
                ...
                <form className="mb-3 grid grid-cols-3">
                    <input className="col-span-2 rounded-md text-sm px-3 py-1" placeholder="Search Name" name="search" onChange={handleSearch} />
